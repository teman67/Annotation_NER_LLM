...
if model_provider == "OpenAI":
    model = st.sidebar.selectbox("OpenAI model", ["gpt-4o", "gpt-4", "gpt-3.5-turbo"])
else:
...
chunk_size = st.sidebar.slider("Chunk size (in characters)", 500, 4000, 1000, step=100)
max_tokens = st.sidebar.slider("Max tokens per response", 200, 4000, 1000, step=100)
...
if st.session_state.annotation_complete and st.session_state.annotated_entities:
    st.header("📝 Edit Annotations")
    
    # Convert to DataFrame for editing
    df_entities = pd.DataFrame(st.session_state.annotated_entities)
    
    # Use data editor with a unique key
    ...
    st.session_state.annotated_entities = edited_df.to_dict(orient="records")
