    ...
    for batch_idx, entity_batch in enumerate(entity_batches):
        ...
        with st.spinner(f"LLM evaluating batch {batch_idx + 1}..."):
            ...
            for eval_result in batch_evaluations:
                eval_result['entity_index'] += batch_idx * batch_size
...
def apply_evaluation_recommendations(entities, evaluations, selected_indices):
    ...
    for eval_idx in selected_indices:
        if eval_idx < len(evaluations):
            ...
            entity_idx = evaluation['entity_index']
            
            if entity_idx < len(updated_entities):
                recommendation = evaluation['recommendation']
                
                if recommendation == 'delete':
                    # Mark for deletion (we'll filter these out later)
                    updated_entities[entity_idx]['_to_delete'] = True
                    changes_made.append(f"Marked entity '{evaluation['current_text']}' for deletion")
                    
                elif recommendation == 'change_label' and evaluation.get('suggested_label'):
                    # Change the label
                    old_label = updated_entities[entity_idx]['label']
                    updated_entities[entity_idx]['label'] = evaluation['suggested_label']
                    changes_made.append(f"Changed '{evaluation['current_text']}' from '{old_label}' to '{evaluation['suggested_label']}'")
    
    # Filter out entities marked for deletion
    final_entities = [entity for entity in updated_entities if not entity.get('_to_delete', False)]
    
    # Clean up the _to_delete flag from remaining entities
    for entity in final_entities:
        if '_to_delete' in entity:
            del entity['_to_delete']
    
    return final_entities, changes_made
...
if st.session_state.get('evaluation_complete') and st.session_state.get('evaluation_results'):
    ...
    with col_b:
        accuracy = summary['correct_count'] / summary['evaluated_entities'] * 100 if summary['evaluated_entities'] > 0 else 0
        st.metric("Correct", summary['correct_count'], delta=f"{accuracy:.1f}%")
    with col_c:
        st.metric("Suggested Changes", summary['change_recommendations'])
    with col_d:
        st.metric("Suggested Deletions", summary['delete_recommendations'])
    
    # Display evaluation results table
    st.markdown("### 📋 Evaluation Results & Recommendations")
    
    # Convert evaluation results to DataFrame for display
    eval_df_display = []
    for i, eval_result in enumerate(st.session_state.evaluation_results):
        entity_idx = eval_result.get('entity_index', i)
        
        eval_df_display.append({
            'ID': i,
            'Entity Index': entity_idx,
            'Text': eval_result.get('current_text', ''),
            'Current Label': eval_result.get('current_label', ''),
            'Is Correct': '✅' if eval_result.get('is_correct', False) else '❌',
    ...
    actionable_evals = [
        (i, eval_result) for i, eval_result in enumerate(st.session_state.evaluation_results)
        if eval_result.get('recommendation') in ['change_label', 'delete']
    ]
    ...
    if actionable_evals:
        ...
        selection_options = []
        for i, (eval_idx, eval_result) in enumerate(actionable_evals):
            action = "DELETE" if eval_result.get('recommendation') == 'delete' else f"CHANGE to '{eval_result.get('suggested_label')}'"
            option_text = f"[{eval_idx}] '{eval_result.get('current_text', '')}' → {action}"
            ...
            if st.button("✅ Apply Selected", disabled=not selected_recommendations, key="apply_recommendations_btn"):
                if selected_recommendations:
                    try:
                        ...
                        # Clear evaluation results to allow re-evaluation
                        st.session_state.evaluation_results = []
                        st.session_state.evaluation_complete = False
                        st.session_state.evaluation_summary = {}
        ...
        st.success("🎉 No actionable recommendations - all annotations look good!")
